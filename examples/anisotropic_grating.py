...
def simulate_grating(
    ...
    # Get permittivity tensor elements for the anisotropic medium and add spatial
    # dimensions.
    ...
    permittivities_grating = [
        fmmax.interpolate_permittivity(
            permittivity_solid=pt,
            permittivity_void=pg,
    ...
    primitive_lattice_vectors = fmmax.LatticeVectors(
        u=fmmax.X * pitch_nm, v=fmmax.Y * pitch_nm / (2 * approximate_num_terms)
    )
    ...
    # Perform the appropriate layer eigensolve for each layer in the stack. Note that
    # the ordering of tensor elements in the `permittivities_grating` and
    # `permittivities_substrate` must must match those in the arguments of
    # `eigensolve_patterned_anisotropic_media`.
    layer_solve_results = [
        ...
        fmmax.eigensolve_anisotropic_media(
            wavelength_nm,
            in_plane_wavevector,
            primitive_lattice_vectors,
            *permittivities_grating,  # type: ignore[arg-type]
            expansion=expansion,  # type: ignore[misc]
            formulation=fmmax.Formulation.FFT,  # type: ignore[misc]
        ),
        fmmax.eigensolve_anisotropic_media(
            wavelength_nm,
            in_plane_wavevector,
    ...
    _, backward_amplitude_0_start = fmmax.colocate_amplitudes(
    ...
    incident_power, reflected_power = fmmax.amplitude_poynting_flux(
    ...
    transmitted_power, _ = fmmax.amplitude_poynting_flux(
