...
def layer_integrated_absorption(
    forward_amplitude_start: jnp.ndarray,
    backward_amplitude_end: jnp.ndarray,
    layer_solve_result: fmm.LayerSolveResult,
    layer_thickness: jnp.ndarray,
    ...
    """Computes the absorption in a layer integrated over the z dimension.

    The resulting absorption array should be averaged to obtain a quantity with units
    that match the flux computed e.g. by `amplitude_poynting_flux`.
    ...
    Args:
        ...
        layer_znum: The number of gridpoints in the z-direction for the layer.
        grid_shape: The shape of the xy real-space grid.
        brillouin_grid_axes: Optional, wpecifies the two axes of the amplitudes
            corresponding to the Brillouin zone grid. When given, the fields of a
            supercell are computed by Brillouin zone integration prior to calculation
            of absortion.

    Returns:
        An array giving the total optical absorption in the layer.
    """
    if brillouin_grid_axes is None:
        forward_amplitude_start = forward_amplitude_start[jnp.newaxis, jnp.newaxis, ...]
        backward_amplitude_end = backward_amplitude_end[jnp.newaxis, jnp.newaxis, ...]
        brillouin_grid_axes = (0, 1)

    num_unit_cells = (
        forward_amplitude_start.shape[brillouin_grid_axes[0]],
        forward_amplitude_start.shape[brillouin_grid_axes[1]],
    )

    z_offsets = jnp.linspace(0, layer_thickness, layer_znum)
    dz = z_offsets[1] - z_offsets[0]

    def fields_at_location(z_offset: jnp.ndarray) -> Tuple[Field, Field]:
        # Compute fields at given location, performing Brillouin zone integration.
        assert z_offset.shape == ()
        forward_amplitude, backward_amplitude = colocate_amplitudes(
            forward_amplitude_start=forward_amplitude_start,
            backward_amplitude_end=backward_amplitude_end,
            z_offset=z_offset,
        ...
        efield_fourier, hfield_fourier = fields_from_wave_amplitudes(
            forward_amplitude=forward_amplitude,
            backward_amplitude=backward_amplitude,
            layer_solve_result=layer_solve_result,
        )
    ...
    def scan_fn(
        carry: Tuple[jnp.ndarray, Tuple[Field, Field]],
        z_offset: jnp.ndarray,
    ) -> Tuple[Tuple[jnp.ndarray, Tuple[Field, Field]], Tuple[()],]:
        # Compute absorption at given location from divergence of the Poynting vector.
        assert z_offset.shape == ()
        absorption, (efield_prev, hfield_prev) = carry
        efield, hfield = fields_at_location(z_offset)
        stacked_efield = tuple(
            jnp.stack([f0, f1], axis=-2) for f0, f1 in zip(efield_prev, efield)
        )
        stacked_hfield = tuple(
        ...
        assert absorption_at_offset.shape[-2] == 1
        absorption_at_offset = jnp.squeeze(absorption_at_offset, axis=-2)
        absorption -= absorption_at_offset * dz
        return (absorption, (efield, hfield)), ()

    shape = tuple(
        size
        for (idx, size) in enumerate(forward_amplitude_start.shape)
        if idx not in brillouin_grid_axes
    )
    absorption_shape = shape[:-2] + (
        grid_shape[0] * num_unit_cells[0],
        grid_shape[1] * num_unit_cells[1],
        shape[-1],
    )
    initial_absorption = jnp.zeros(absorption_shape)
    initial_field = fields_at_location(z_offsets[0])

    (absorption, _), _ = jax.lax.scan(
        scan_fn,
        (initial_absorption, initial_field),
        z_offsets[1:],
    )
...
def _poynting_vector_divergence(
    efield: Field,
    hfield: Field,
    primitive_lattice_vectors: basis.LatticeVectors,
    num_unit_cells: Tuple[int, int],
    ...
    """Computes Poynting vector divergence given the real-space fields."""
    shape = efield[0].shape
    u_magnitude = jnp.sqrt(jnp.sum(primitive_lattice_vectors.u**2))
    v_magnitude = jnp.sqrt(jnp.sum(primitive_lattice_vectors.v**2))
    # Note the magnitude squared of the vectors u and v here, which seems to be needed.
    # This is probably due to an an error elsewhere, where the fields are improperly
    # normalized---either having an extra factor of the unit cell dimension, or
    # neglecting to include such a factor when it is actually needed.
    # TODO: investigate this normalization.
    du = num_unit_cells[0] / shape[-4] * v_magnitude**2
    dv = num_unit_cells[1] / shape[-3] * u_magnitude**2

    ex, ey, ez = efield
    hx, hy, hz = hfield

    sx = (ey * hz.conj() - ez * hy.conj()).real
    sy = (ez * hx.conj() - ex * hz.conj()).real
    sz = (ex * hy.conj() - ey * hx.conj()).real

    dsx_du = _periodic_diff(sx, n=1, axis=-4) / du
    dsy_du = _periodic_diff(sy, n=1, axis=-4) / du
    dsx_dv = _periodic_diff(sx, n=1, axis=-3) / dv
    dsy_dv = _periodic_diff(sy, n=1, axis=-3) / dv
    dsz_dz = jnp.diff(sz, n=1, axis=-2) / dz

    dsx_du = 0.5 * (dsx_du[..., :-1, :] + dsx_du[..., 1:, :])
    dsy_du = 0.5 * (dsy_du[..., :-1, :] + dsy_du[..., 1:, :])
    dsx_dv = 0.5 * (dsx_dv[..., :-1, :] + dsx_dv[..., 1:, :])
    dsy_dv = 0.5 * (dsy_dv[..., :-1, :] + dsy_dv[..., 1:, :])

    u_x = primitive_lattice_vectors.u[..., 0]
    u_y = primitive_lattice_vectors.u[..., 1]
    v_x = primitive_lattice_vectors.v[..., 0]
    v_y = primitive_lattice_vectors.v[..., 1]

    dsx_dx = dsx_du * u_x + dsx_dv * v_x
    dsy_dy = dsy_du * u_y + dsy_dv * v_y
    return dsx_dx + dsy_dy + dsz_dz


def _periodic_diff(x: jnp.ndarray, n: int, axis: int) -> jnp.ndarray:
    """Compute the finite difference gradient with periodic boundary conditions."""
    (axis,) = utils.absolute_axes((axis,), x.ndim)
    slices = tuple(
        slice(0, dim) if idx != axis else (0,) for idx, dim in enumerate(x.shape)
    )
    append = x[slices]
    return jnp.diff(x, n=n, axis=axis, append=append)
